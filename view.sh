...
docs = [ROOT / "README.md"] + sorted(ROOT.glob("deploy/*.md"))
docs = [d for d in docs if d.exists()]

# Code files worth showing on screen, rendered with syntax highlighting.
CODE = [ROOT / "app/services/renderer.py", ROOT / "app/services/ranking.py",
        ROOT / "app/services/sessions.py", ROOT / "scripts/demo.py"]
CODE = [c for c in CODE if c.exists()]

def label_for(p: Path) -> str:
    # deploy/README.md must not collide with the top-level README.md
    if p.name == "README.md":
        return "README" if p.parent == ROOT else f"{p.parent.name}-README"
    return p.stem
